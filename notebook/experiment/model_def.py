...
class MRIUnetTrial(PyTorchTrial):
    ...
    def download_data(self, data_config, data_dir):
        pach_data = data_config["pachyderm"]
        host = pach_data["host"]
        port = pach_data["port"]
        repo = pach_data["repo"]
        project = pach_data["project"]
        branch = pach_data["branch"]
        token = pach_data["token"]
        files = download_pach_repo(pachyderm_host=host, pachyderm_port=port, repo=repo, project=project,
                                  branch=branch, token=token, root=data_dir)
